...
export class ResourcesComponent implements OnInit {
  ...
  private getEffectiveDate(resource: ServicePatientResource): Date {
    ...
    switch (resource.resourceJson.resourceType) {
      ...
      case 'QuestionnaireResponse':
        return resource.effectiveDate = DateHelper.parse(resource.resourceJson.authored);
      case 'Procedure':
  ...
  private getEncounterDescription(resource: ServicePatientResource) {
    ...
    let codeTerm = this.getCodeTerm((resource.resourceJson.reason && resource.resourceJson.reason.length > 0) ? resource.resourceJson.reason[0] : null);
    if (codeTerm)
      description += codeTerm;
